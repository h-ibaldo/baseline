<script lang="ts">
	...
	let formData = {
		...
		description: '',
		category: 'custom',
		content: JSON.stringify({ type: 'text', text: 'Your content here' }, null, 2),
		isPublic: false
	};
...
<svelte:head>
	<title>Create Content Block - LineBasis Admin</title>
</svelte:head>
		...
		<form on:submit|preventDefault={handleSubmit}>
			...
			<div class="section">
				<div class="section-header">
					...
					<button type="button" class="btn-secondary" on:click={formatJSON}>Format JSON</button>
				</div>
				...
				<div class="form-group">
					...
					{:else}
						<p class="field-help">Define block structure as JSON (e.g., {`{ "type": "text", "text": "..." }`})</p>
					{/if}
				</div>

				<div class="examples">
					<details>
						<summary>Example Structures</summary>
						<pre>{`Text Block:
{ "type": "text", "text": "Your text here", "style": "body" }

Button Block:
{ "type": "button", "text": "Click me", "href": "/page", "variant": "primary" }

Image Block:
{ "type": "image", "src": "/image.jpg", "alt": "Description" }`}</pre>
					</details>
				</div>
			</div>
